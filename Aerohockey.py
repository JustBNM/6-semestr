...
# настройки шайбы
# Насколько будет увеличиваться скорость шайбы с каждым ударом
BALL_SPEED_UP = 1.05
# Максимальная скорость шайбы
BALL_MAX_SPEED = 40
# радиус шайбы
BALL_RADIUS = 30
...
def spawn_ball():
    global BALL_X_SPEED
    # Выставляем шайбу по центру
    c.coords(BALL, WIDTH / 2 - BALL_RADIUS / 2,
             HEIGHT / 2 - BALL_RADIUS / 2,
             WIDTH / 2 + BALL_RADIUS / 2,
             HEIGHT / 2 + BALL_RADIUS / 2)
    # Задаем шайбе направление в сторону проигравшего игрока,
...
# добавим глобальные переменные для скорости движения шайбы
# по горизонтали
BALL_X_CHANGE = 20
# по вертикали
BALL_Y_CHANGE = 0
...
def move_ball():
    # определяем координаты сторон шайбы и его центра
    ball_left, ball_top, ball_right, ball_bot = c.coords(BALL)
    ball_center = (ball_top + ball_bot) / 2

    # вертикальный отскок
    # Если мы далеко от вертикальных линий - двигаем шайбу
    if ball_right + BALL_X_SPEED < right_line_distance and \
            ball_left + BALL_X_SPEED > PAD_W:
        c.move(BALL, BALL_X_SPEED, BALL_Y_SPEED)
    # Если шайба касается своей правой или левой стороной границы поля
    elif ball_right == right_line_distance or ball_left == PAD_W:
        # Проверяем правой или левой стороны мы касаемся
        if ball_right > WIDTH / 2:
            # Если правой, то сравниваем позицию центра шайбы
            # с позицией правой ракетки.
            # И если шайба в пределах ракетки делаем отскок
            ...
            if c.coords(LEFT_PAD)[1] < ball_center < c.coords(LEFT_PAD)[3]:
                ...
                update_score("right")
                spawn_ball()
    # Проверка ситуации, в которой шайба может вылететь за границы игрового поля.
    # В таком случае просто двигаем ее к границе поля.
    else:
        if ball_right > WIDTH / 2:
...
def move_pads():
    # создадим словарь, где ракетке соответствует ее скорость
...
def main():
    move_ball()
    move_pads()
    # вызываем саму себя каждые 20 миллисекунд
    root.after(20, main)
